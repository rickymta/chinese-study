...
- **App/module nào?** (service nào: `identity-service` / `chinese-backend` / `gateway`; app nào: `apps/chinese` học viên hay màn quản trị nội dung; module `pinyin` / `vocabulary` / `srs` / `hanzi` / `lessons`...)
...
4. Ghi nhận ràng buộc dự án (MUI v9, Npgsql `timestamptz` UTC, DDD 4 lớp, `@af/*` workspace source, phân quyền cục bộ...).
