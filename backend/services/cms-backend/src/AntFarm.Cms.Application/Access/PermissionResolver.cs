...
/// <summary>
/// Nguồn sự thật DUY NHẤT của quyền hiệu lực (R-W1) — join users → user_roles → role_permissions
/// trên DB `af_cms`, KHÔNG suy quyền từ claim JWT. Cache 60 giây theo userId để
...
/// </summary>
public sealed class PermissionResolver(ICmsDbContext db, TimeProvider timeProvider) : IPermissionResolver
{
    ...
    /// <summary>Bỏ cache của một user — dùng khi vai trò vừa đổi (<c>UserAdminService.SetRolesAsync</c>) để quyền có hiệu lực NGAY thay vì chờ tối đa 60 giây.</summary>
