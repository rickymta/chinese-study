...
| 21 | Múi giờ ở Hồ sơ (M4) | Thêm → Hồ sơ → Thông tin → chạm ô Múi giờ | Sheet liệt kê múi giờ của máy (`getAvailableTimezones`, không phải danh sách dự phòng 36 mục); gõ "ho chi" ra `Asia/Ho_Chi_Minh`; ghim "Múi giờ của máy" đúng | ☐ |
| 22 | Đổi mật khẩu trên máy thật (M4) | Đăng nhập app + web cùng tài khoản → app đổi mật khẩu | Web bị đăng xuất, app vẫn dùng (`currentSessionKept=true`) | ☐ |
| 23 | Làm mới tổng quan khi app trở lại foreground (M5) | Mở app ở trang chủ → ôn/học trên web cùng tài khoản → đưa app ra nền rồi mở lại | Số liệu/huy hiệu "Ôn tập" cập nhật không cần kéo (`AuthLifecycleObserver.onResumed`); múi giờ máy ≠ hồ sơ ⇒ dòng nhắc dưới tiêu đề (`flutter_timezone` trên máy thật) | ☐ |
| 24 | Pinyin & luyện thanh (M7) | Thêm → Pinyin: tab Bảng chạm "ma" → "Nghe lần lượt"; tab Luyện → Bắt đầu (20 câu) → chọn thanh → "Tiếp"; làm hết → "Đã lưu · ngày học"; tắt mạng trước câu cuối → "Gửi lại" | 4 chữ đọc lần lượt cách ~0,6 s, dừng khi vuốt đóng sheet; câu mới tự đọc khi bấm "Tiếp" (iOS phát được vì trong thao tác chạm), câu đúng tự chuyển sau 1,2 s thì phải bấm "Nghe"; nút thanh cao ≥ 64, bảng không tràn ngang ở 360 px; `learning.tone_drill_sessions` +1, "Gửi lại" không tạo phiên thứ hai. Đã kiểm trên web-dev (Chrome headless, speechSynthesis giả) 18/09/2026 — máy thật chưa | ☐ |
| 25 | Tra từ (M8) | Thêm → Tra từ: gõ `ai` (bàn phím máy, có gợi ý/tự sửa tắt) → chạm 爱 → chạm ô chữ 爱 → quay lại hai lần | Kết quả hiện sau ~300 ms mỗi lần ngừng gõ; bàn phím có nút "Tìm"; quay lại vẫn đúng kết quả + vị trí cuộn; nút nghe ở trang từ/chữ đọc được; "Thêm vào ôn tập" ⇒ chip "Chờ học" + huy hiệu "Ôn tập" đổi. Đã kiểm trên web-dev (Chrome headless) 18/09/2026 — máy thật chưa | ☐ |
| 26 | Bài học & quiz (M9) | Bài học → thẻ "Bài tiếp theo" (Chào hỏi) → tab Nội dung chạm chữ 你好 / "Nghe cả đoạn"; tắt công tắc Pinyin; tab Từ vựng chạm 你 → quay lại; tab Quiz → Bắt đầu → làm 7 câu (tắt mạng trước khi Nộp → Thử lại) → đạt → "Ôn tập ngay"; back hệ thống giữa chừng lượt | Ruby pinyin trên chữ không chồng dòng ở cỡ chữ hệ thống lớn; câu nghe tự đọc khi bấm Tiếp/Trước (iOS phát được vì trong thao tác chạm); Nộp/Thử lại cùng `clientAttemptId` ⇒ `learning.quiz_attempts` 1 dòng; đạt lần đầu ⇒ thẻ `srs_cards` source=lesson, huy hiệu Ôn tập đổi; back giữa chừng hỏi "Thoát lượt làm?". Đã kiểm trên web-dev (Chrome headless, speechSynthesis giả) 18/09/2026 — máy thật chưa | ☐ |
