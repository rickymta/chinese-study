...
mobile/
  ...
  apps/chinese/               # af_chinese — bundle xyz.antfarms.chinese, tên "AntFarm Trung"
    ...
    lib/features/profile/ (hồ sơ 4 tab) · features/srs/ (learning-settings, ôn thẻ + outbox bền) · features/dictionary/ (M8: tìm, chi tiết từ/chữ)
    ...
    lib/features/pinyin/ (M7: hướng dẫn, bảng âm tiết + sheet 4 thanh, luyện nghe–chọn thanh; domain port generateDrill/drillTypes)
    lib/features/lessons/ (M9: danh sách bài, bài học 3 tab, quiz; domain port inlineZh/quizScore/shuffle)
...
- `/on-tap/phien` (`ReviewSessionPage`, root navigator + `AuthGate`, ẩn bottom nav): `SessionHeader`, `Flashcard` (chạm thẻ cũng lật; chữ ≥ 4 ký tự hạ cỡ), `RatingBar` 4 nút ≥ 56 px hai dòng, khoá 300 ms + `HapticFeedback.selectionClick`, `durationMs` đo bằng `Stopwatch`, `StickyActionBar`; tự đọc khi thẻ hiện nếu `autoPlayAudio` (khi chấm đọc thẻ kế ngay trong thao tác chạm — iOS), huỷ đọc khi rời màn; "Xem chi tiết" ⇒ `showWordDetailSheet` (`closeOnBarrier: true`, chỉ đọc) dùng `WordDetailView(linkCharacters: false)` + `AddToSrsButton` (`features/dictionary/`; trong sheet ô "Chữ trong từ" KHÔNG dẫn đi — sheet chỉ để đọc nhanh rồi về thẻ, trang `/tu-dien/:id` của M8 thì dẫn sang chữ). Tải thêm khi còn ≤ 5 thẻ với `skipNew` khi có lượt chấm chưa chắc tới server; lô không thêm gì mà outbox còn ⇒ **không** gọi lại `/srs/queue` ngay (chờ outbox đổi/lượt chấm kế — tương đương effect theo deps của web). Hết thẻ ⇒ `SessionSummary` (vẫn hiện banner chờ gửi nếu còn). Rời khi `unsentCount > 0` ⇒ `PopScope` + hỏi.
...
## Pinyin & luyện thanh (M7)

- `/pinyin?tab=huong-dan|bang|luyen&che-do=mot|cap` (`features/pinyin/`, port `features/pinyin/` web): 3 tab, tab khởi đầu + chế độ từ query lúc mở rồi state cục bộ (RM-L6, đọc trong `didChangeDependencies` vì `GoRouterState.of` phụ thuộc InheritedWidget); `VoiceMissingNotice` chung ở đầu trang khi `noVoice`/`unsupported`; nút "Cài đặt giọng đọc" ⇒ `/ho-so?tab=giao-dien`.
- `domain/` port thuần kèm test cùng số ca web (11 + 6) **và so chéo**: `generate_drill.dart` (`buildTonePools`, `kTonePairCombos` 15 tổ hợp không 3-3, `generateDrill({mode, chart, focus, count, random})` nhận `Random`), `drill_types.dart` (`computeResponseMs` kẹp 600000 ⇒ null, `summarizeByTone`, `buildSubmitRequest` ISO có `Z`), `normalizeToneStats` trong `data/models.dart` (RK41: thiếu/`null` ⇒ 0/null, `byTone` đủ 4 khoá). Fixture `test/fixtures/generate_drill_crosscheck.json` sinh bằng Node từ **chính `generateDrill.ts`** với LCG seed (`SeededLcg implements Random` cùng công thức `seeded()` web) — 9 ca Dart ⇄ TS trùng từng (âm tiết, thanh, chữ).
- Provider: `pinyinChartProvider`/`pinyinGuideProvider` autoDispose + `ref.keepAlive()` CHỈ sau khi tải thành công (học liệu tĩnh, ETag phía server — tương đương `staleTime: Infinity`; lỗi 503 không bị giữ, "Thử lại"/mở lại tab tải mới); `toneStatsByUserProvider` family theo id người dùng (như tổng quan), `ref.invalidateToneStats()` + `ref.invalidateProgressOverview()` sau khi nộp. **Riverpod 3 tạm dừng subscription của widget không hiển thị** (nhánh Trang chủ offstage trong `IndexedStack`) ⇒ tổng quan chỉ tải lại khi quay về Trang chủ — không gọi API thừa (widget test M7 chốt hành vi này).
- Bảng: hàng vận mẫu theo nhóm (hàng tiêu đề nhóm), cột Ø + thanh mẫu; chip lọc 2 hàng cuộn ngang (mặc định `Môi` — D33); hàng tiêu đề + cột đầu dính tự dựng bằng 2 cặp `ScrollController` đồng bộ (không thêm `two_dimensional_scrollables`); ô 48×44; chỉ vùng bảng cuộn hai chiều. Chạm ô ⇒ `showSyllableSheet` (`closeOnBarrier: true` — chỉ đọc) 4 dòng thanh + "Nghe lần lượt" (600 ms, `runId` huỷ khi đóng/rời).
- Luyện: `DrillSetup` (`SegmentedButton`, thanh tập trung từ `recommendedFocus`, khoá khi chưa `ready`/học liệu lỗi) + `ToneStatsCard`; `DrillRunner` (nút Nghe 64 px, `ToneButtons` ≥ 64 px cố định ở nửa dưới, `tone_pair` hai hàng; chấm ngay, đúng tự sang sau 1,2 s (`Timer`), sai bấm "Tiếp" ở `StickyActionBar`; `responseMs` bằng `Stopwatch` từ lúc phát xong lần đầu; **tự phát câu mới chỉ khi chuyển bằng thao tác chạm** — RM-L5/iOS); `clientSessionId = uuidV4()` sinh lúc bắt đầu, giữ bộ nhớ (RM-L2), "Gửi lại" dùng cùng id (server 200), 400/422 ⇒ thông điệp server + chỉ "Làm bài mới". Trạng thái bài sống qua đổi tab (`AutomaticKeepAliveClientMixin`); rời TRANG khi đang làm ⇒ `PopScope` + `showAfConfirm` "Bỏ bài đang làm?" (RM-L7).
- Test: domain 37 ca (kể cả so chéo), widget 10 ca (3 tab, sheet "ma" 4 thanh + nghe lần lượt, deep link `?tab=luyen&che-do=cap`, 20 câu nộp 201 + thống kê làm mới, mất mạng ⇒ Gửi lại, 422, rời trang hỏi, không giọng, 503, tối + 360×740 + 1,3×).
- Chạy thật web-dev 18/09/2026 (Chrome headless 390×844, `speechSynthesis` giả cài qua CDP `Page.addScriptToEvaluateOnNewDocument`, tài khoản mới): Hướng dẫn hiện 8 chủ đề thật, "Nghe 妈" đọc; Bảng lọc Môi không tràn ngang, sheet "ma" 4 thanh, "Nghe lần lượt" đọc 妈 麻 马 骂 rồi tự về "Nghe lần lượt"; Luyện 20 câu ⇒ POST 201, `learning.tone_drill_sessions` +1 / `tone_drill_answers` +20 / `study_events` kind `tone_drill` local_date theo múi giờ hồ sơ; nộp lại cùng body bằng curl ⇒ 200 cùng id; tắt mạng trước câu cuối ⇒ "Chưa lưu được… Gửi lại" ⇒ bật mạng ⇒ Gửi lại cùng `clientSessionId` ⇒ 201, DB đúng 2 phiên không trùng; "Xem thống kê" ⇒ thẻ "1 phiên · 20 câu"; về Trang chủ ⇒ `/progress/overview` tải lại, thẻ Thanh điệu "25% nghe đúng thanh · 40 câu". Android/iOS CHƯA VERIFY (VERIFY-DEVICE #24).
- Mẹo lái Flutter web bằng CDP (đợt sau dùng lại): đăng nhập TRƯỚC khi bật semantics (bật rồi thì lớp `<input>` a11y nuốt click, chữ không vào ô); `Emulation.setDeviceMetricsOverride` và script inject chỉ sống trong một phiên CDP ⇒ đặt lại mỗi lần chạy.

## Tra từ (M8)

- `features/dictionary/` (port `features/dictionary/` web): `data/models.dart` thêm `WordSummary`/`SearchResponse`/`CharacterDetail` (viết tay `fromJson`, thiếu khoá = null/mặc định, dòng thiếu `id`/`simplified` bị bỏ); `data/dictionary_api.dart` `searchWords` (`q` trim, `hsk`, `page`, `pageSize` 1..100), `getWord(skipErrorRedirect:)` (sheet phiên ôn `true`; trang `false` ⇒ 404 ⇒ `/404` qua interceptor, giữ nút quay lại), `getCharacter` (`Uri.encodeComponent`, go_router giải mã `pathParameters`); `domain/search_errors.dart` port `QueryErrorAlert` (503 ⇒ "Từ điển chưa được nạp dữ liệu…" không Thử lại; 400 ⇒ thông điệp đầu trong `details`).
- Provider: `wordDetailProvider` (sheet) và `wordPageProvider` (trang) tách nhau vì khác `skipErrorRedirect`, đều autoDispose + theo `userScopeProvider`; `AddToSrsButton` làm mới cả hai + tóm tắt SRS + tổng quan. `characterDetailProvider` autoDispose (học liệu chung). **`dictionarySearchProvider`** (`Notifier`, KHÔNG autoDispose, `build` watch `userScopeProvider` ⇒ đổi người tự đặt lại): từ khoá/lọc/các trang đã nối (cuộn vô hạn, **khử trùng theo `id`** khi nối), "thế hệ" bỏ phản hồi cũ khi đổi từ khoá; **dừng tải** (`endReached`) khi server trả trang ít hơn `pageSize`/rỗng (`SearchResponse.rawCount` — số dòng THÔ, vì dòng hỏng bị `fromJson` lược làm `items.length` không bao giờ đuổi kịp `totalCount`) hoặc trang kế không thêm dòng mới; giữ state trong nhánh ⇒ quay lại từ chi tiết (hoặc mở lại từ "Thêm") không gọi lại API — Riverpod 3 tạm dừng subscription của widget bị che nên autoDispose sẽ mất danh sách.
- `/tu-dien?q=` (`DictionarySearchPage`): ô tìm dính đầu (`TextInputAction.search`, `maxLength` 64, nút xoá), debounce 300 ms (`Timer`), `q` đọc từ URL **một lần lúc dựng** (cắt 64 ký tự, không ghi lại URL) — **lưu ý M9+:** `go('/tu-dien?q=B')` khi trang đã nằm trong nhánh KHÔNG đổi từ khoá; muốn mở với từ khoá mới phải `push` (trang mới) hoặc chuyển sang theo dõi `GoRouterState`; chip `FilterChip` "HSK 1"; `ListView.builder` + `AlwaysScrollableScrollPhysics` (kéo-làm-mới cả khi 503/rỗng), tải trang kế khi dựng tới dòng còn ≤ 5 (`addPostFrameCallback`), cuối danh sách: vòng xoay / "Thử lại" trang kế / dòng ghi công `SourceAttribution` (chạm ⇒ `/giay-phep`); đang tải từ khoá mới ⇒ danh sách cũ mờ 0,6. `WordListTile` ≥ 64 px: chữ 28, pinyin dấu đậm + Hán Việt in hoa, ≤ 2 nghĩa một dòng, `MeaningStatusChip(triggerMode: longPress)` (giữ lâu mới hiện tooltip — chạm vào chip vẫn mở từ; chip chiếm ~104 px mép phải ở 360 px, rộng hơn khi chữ 1.3×). Vị trí cuộn giữ nhờ trang tìm còn trong ngăn xếp nhánh (`push` chi tiết lên trên).
- `/tu-dien/:id` (`WordDetailPage`): `WordDetailView` (ô "Chữ trong từ" `InkWell` ⇒ `/tu-dien/chu/:hanzi`; ô cao tối thiểu 88, không cố định — tràn 6 px ở 1.3×) + `SourceAttribution`; `/tu-dien/chu/:hanzi` (`CharacterDetailPage`): chữ 96 + `SpeakButton`, Cách đọc/Hán Việt (âm đầu đậm)/Số nét/Bộ thủ "爪 (bộ số 87)"/Phồn thể (`zh-TW`), "Từ có chữ này" (`WordListTile` ≤ 20); nút "Luyện viết chữ này" để M10. `DictionaryBackButton`: pop được thì pop, deep link ⇒ `go('/tu-dien')`. Route phẳng trong nhánh "Thêm" (`/tu-dien/chu/:hanzi` khai trước `/tu-dien/:id`), `push` giữ bottom nav.
- Test: parse fixture `dictionary_search.json`/`dictionary_word.json` (không `srs`)/`dictionary_character.json` + `search_errors` (8 ca domain mới); widget 9 ca (`FakeDictionaryServer`): lộ trình 500 từ + cuộn ⇒ trang 2 + quay lại giữ vị trí/không gọi lại API + mở lại từ "Thêm" giữ state; gõ "ai" debounce đúng 1 lời gọi ⇒ chi tiết ⇒ Thêm vào ôn tập (`POST /srs/cards {wordIds:[w1]}`) ⇒ "Chờ học" + `/srs/summary` gọi lại + huy hiệu 1 ⇒ ô chữ ⇒ trang chữ ⇒ từ chứa chữ ⇒ quay lại 3 lần; 7 cách gõ đều ra 爱 + "zzz" rỗng; chip HSK + trang 2 lỗi ⇒ Thử lại; deep link `?q=yêu` và `/tu-dien/chu/%E7%88%B1` + quay lại về `/tu-dien`; id lạ ⇒ `/404`, 2 ký tự ⇒ 400 tại chỗ; 503 ⇒ không Thử lại ⇒ kéo-làm-mới sau khi nạp; sheet phiên ôn không dẫn đi; tối + 360×740 + 1,3× ba trang.
- Review M7 làm cùng đợt: `DrillSetup` hiện "Đang kiểm tra giọng đọc…" khi `loading` (test mới); `DrillTab` giữ `_chart` từ lúc bắt đầu (không `chart.value!`; mất ⇒ về thiết lập + toast); `PinyinChartView._follow` kẹp offset vào `maxScrollExtent` bên nhận; test nộp bài: sửa bình luận sai + ca "500 một lần ⇒ Gửi lại cùng `clientSessionId`, server 1 phiên" + "rớt mạng khi Gửi lại ⇒ Gửi lại tiếp cùng id"; `kTonePairCombos` `List.unmodifiable`; `VERIFY-DEVICE.md` sắp lại #23/#24, thêm #25.

## Bài học & quiz (M9)

- `features/lessons/` (port `features/lessons/` web): `data/models.dart` (`LessonSummary`/`LessonDetail`/`LessonBlock` **sealed** `TextBlock|DialogueBlock|GrammarBlock|TipBlock|UnknownBlock` — kiểu lạ không crash; `QuizQuestion` KHÔNG có đáp án; `QuizResult`, `QuizAttemptSummary`; viết tay `fromJson`, thiếu khoá = null/mặc định), `data/lessons_api.dart` (`GET /lessons`, `GET /lessons/{slug}`, `POST /lessons/{id}/start`, `POST|GET /lessons/{id}/quiz-attempts?limit=` 1..20), `domain/` port thuần kèm test cùng số ca web: `inline_zh.dart` (`[[chữ Hán|pinyin]]`, token hỏng giữ nguyên văn — 11 ca), `quiz_score.dart` (`scorePercent` floor, `isPassed` số nguyên, `minCorrectToPass`, `countUnanswered`, `firstUnansweredIndex`, `buildAnswers` — 7 ca), `shuffle.dart` (Fisher–Yates + `seededRng` LCG cùng hằng số web, so chéo 3 giá trị seed 42 — 8 ca), `lesson_errors.dart` (503 ⇒ "Học liệu chưa sẵn sàng" không Thử lại; `QUIZ_CHANGED|QUIZ_EMPTY` ⇒ tải lại bài).
- Provider: `lessonsByUserProvider` (family theo id người dùng, autoDispose — deep link thẳng vào bài thì danh sách KHÔNG được tải, invalidate không gọi API thừa), `lessonDetailProvider(slug)` = `AsyncNotifier` autoDispose family có `applyProgress()` để ghi thẳng `progress` server trả sau `start`/nộp quiz (như `setQueryData` web, không GET lại), `quizAttemptsProvider(lessonId)`, `displayPrefsProvider` (công tắc Pinyin/Nghĩa Việt, khoá `af.chinese.lesson.showPinyin|showVi` trong shared_preferences, mặc định bật, chọn trước khi đọc xong kho thì lựa chọn thắng). Sau nộp quiz `ref.afterQuizSubmitted()` invalidate đúng danh sách web: lessons, attempts, tóm tắt SRS (huy hiệu "Ôn tập"), tổng quan; `firstCompletion` ⇒ tải lại chi tiết (chip "Đang ôn" ở tab Từ vựng).
- `/bai-hoc` (`LessonListPage`): thẻ "BÀI TIẾP THEO" viền nổi (`nextLessonSlug`, key riêng `kNextLessonCardKey` vì cùng bài xuất hiện hai lần trong một `Column`), "TẤT CẢ BÀI HỌC (n)" theo `orderIndex`, chip Chưa học/Đang học/Hoàn thành {best}% + "Nội dung chưa được duyệt" (tooltip khi chạm), rỗng ⇒ `EmptyState`, kéo-làm-mới. Chạm thẻ ⇒ `go('/bai-hoc/<slug>')` (route PHẲNG trong nhánh — không chồng danh sách; nút quay lại của bài `pop` nếu được, không thì `go('/bai-hoc')`).
- `/bai-hoc/:slug?tab=noi-dung|tu-vung|quiz` (`LessonDetailPage`): tab khởi đầu từ query rồi state cục bộ (RM-L6); 3 tab **keep-alive** (đổi tab không mất đáp án/`clientAttemptId`, "Nghe cả đoạn" không bị cắt); `POST /start` gọi ĐÚNG MỘT LẦN khi `progress == null` (post-frame, giữ id đã gọi); **lệch có chủ đích mobile-first:** phần đầu bài (chip trạng thái/~phút/chưa duyệt, tóm tắt, mục tiêu, `VoiceMissingNotice`, banner "Nên học xong phần Pinyin & thanh điệu trước…" khi `tone-stats.totalAnswered < 40` — lỗi bỏ qua, công tắc Pinyin/Nghĩa Việt) nằm ở ĐẦU tab Nội dung thay vì cố định trên thanh tab như web (360×740 không đủ chỗ). `InlineZhText`: **ruby** tự dựng bằng `WidgetSpan` + `Column` (pinyin dấu 0,7× phía TRÊN chữ Hán 1,2×, `PlaceholderAlignment.bottom`), chạm token ⇒ đọc; tắt Pinyin ⇒ chỉ chữ. Khối hội thoại: `SpeakQueueController` đọc lần lượt (runId, dừng khi rời màn), tên người nói luân phiên `primary`/`secondary`, dòng đang đọc tô nền; ngữ pháp: "MẪU CÂU" viền trái + "VÍ DỤ"; mẹo: dải `secondaryContainer` + tiêu đề theo `variant`. Tab Từ vựng: `LessonWordList` (chữ 30, pinyin dấu + Hán Việt in hoa, ≤ 2 nghĩa, chip "Chưa duyệt"/"Đang ôn" xuống dòng riêng — ở chữ 1,3× hai chip rộng hơn dòng), chạm ⇒ `push('/tu-dien/:id')` — **go_router 18 đặt trang từ điển LÊN TRÊN trang bài trong cùng nhánh "Bài học"** (đã kiểm bằng widget test: nav vẫn ở "Bài học", có nút quay lại ⇒ về bài đúng tab), không chuyển sang nhánh "Thêm" như từng lo ngại; nút "Luyện viết chữ của bài" để M10.
- `QuizRunner` (keep-alive): intro (chip số câu/câu nghe/ngưỡng/điểm cao nhất, cảnh báo không giọng, `QuizHistory` 5 lần — giờ máy `toLocal()` như M6) → câu i/n (lựa chọn nút ≥ 56 px xáo MỘT LẦN lúc bắt đầu lượt; câu nghe: nút Nghe 64 px, tự đọc khi ĐI TỚI câu bằng nút Bắt đầu/Tiếp/Trước nếu `autoPlayAudio` — trong handler chạm, RM-L5; không giọng ⇒ "Hiện chữ") → thanh dính đáy Trước/Tiếp|Nộp (nộp khoá khi còn câu + "Còn N câu chưa trả lời") + "Thoát lượt làm" → kết quả (`QuizResultView`: điểm lớn, Đạt/Chưa đạt "ngưỡng 80% (≥ 6/7)", `firstCompletion` ⇒ "Hoàn thành bài! Đã thêm N từ" khi N > 0 + "Ôn tập ngay" ⇒ `/on-tap`, từng câu: đã chọn/đáp án đúng/lời giải `InlineZhText`, "Làm lại"). `clientAttemptId = uuidV4()` + `startedAt` (UTC, có `Z`) sinh lúc bắt đầu lượt (RM-L2); bấm Nộp lần đầu ⇒ **khoá đáp án** (chạm lựa chọn khác không đổi); lỗi mạng/5xx/409 ⇒ dải "Chưa nộp được bài (…)" + "Thử lại" cùng id ⇒ server 1 lượt; `422 QUIZ_CHANGED` ⇒ toast "Bài vừa được cập nhật — tải lại quiz", `QUIZ_EMPTY` ⇒ "Bài này chưa có câu hỏi." ⇒ invalidate chi tiết, về intro. Rời trang (nút quay lại/back hệ thống) khi đã trả lời ≥ 1 câu ⇒ `PopScope` + hộp "Thoát lượt làm?" (Ở lại/Thoát — RM-L7). Phím tắt 1–4/←/→/Enter của web bỏ (không bàn phím).
- Test: domain 26 ca + parse fixture thật (danh sách 5 bài, `chao-hoi` 8 khối/14 từ/7 câu không đáp án, kết quả sai 28%/đạt lần đầu 14 thẻ/phát lại 0 thẻ, lịch sử, tiến độ sau start) + `DisplayPrefsController`; widget 11 ca (`FakeLessonServer` idempotent theo `clientAttemptId`, chấm theo đáp án từ fixture thật): danh sách/chạm thẻ/quay lại làm mới; start đúng 1 lần + 4 loại khối + ruby + công tắc + chạm chữ đọc + "Nghe cả đoạn" 4 dòng; từ vựng + push từ điển + quay lại; quiz deep link → 7/7 (tự đọc 8 lần: Bắt đầu q1, Tiếp q2 q3 q7, Trước q3 q2 q1, Nghe) → Hoàn thành + 14 từ + huy hiệu 10 + chip Đang ôn ×14 + lịch sử + tổng quan tải lại khi về Trang chủ; sai 28%; offline ⇒ khoá + Thử lại ×2 cùng id ⇒ 1 lượt + 500 rồi Thử lại; 422 ⇒ intro; rời trang hỏi; không giọng ⇒ Hiện chữ; 503; tối + 360×740 + 1,3×. Fixture `test/fixtures/{lessons_list,lesson_chao_hoi,lesson_start,quiz_result_failed,quiz_result_first,quiz_result_replay,quiz_attempts}.json` lấy từ backend thật 18/09/2026.
- Chạy thật web-dev 18/09/2026: Chrome headless 390×844, `speechSynthesis` giả qua CDP, tài khoản mới, backend thật (identity 5281 · chinese 5282 · gateway 5280 · web-dev 3290): danh sách 5 bài + "BÀI TIẾP THEO" Chào hỏi; mở bài ⇒ `POST /start` (`lesson_progress` in_progress) và phản hồi `GET /lessons/chao-hoi` KHÔNG chứa `correctOptionId`; ruby pinyin dấu trên chữ, chạm 你好 ⇒ đọc, "Nghe cả đoạn" đọc 4 dòng lần lượt, tắt Pinyin ⇒ chỉ chữ; tab Từ vựng chạm 你 ⇒ trang "Chi tiết từ" mở TRONG nhánh Bài học (nav vẫn "Bài học"), quay lại về đúng tab; Quiz: Bắt đầu tự đọc 你好, làm sai 7/7 ⇒ 0% "Chưa đạt" + đáp án đúng/lời giải (`quiz_attempts` 1 dòng); "Làm lại" đúng 7/7, tắt mạng rồi Nộp ⇒ dải "Chưa nộp được bài…" + "Thử lại", chạm lựa chọn khác KHÔNG đổi (khoá), bật mạng ⇒ Thử lại cùng `clientAttemptId` ⇒ 100% "Đạt" + "Hoàn thành bài! Đã thêm 14 từ" + "Ôn tập ngay"; DB: `quiz_attempts` 2 dòng = 2 `client_attempt_id` (0/false, 100/true), `srs_cards` 14 thẻ `source=lesson`, `lesson_progress` completed best=100 attempts=2, `study_events` quiz_submit + lesson_complete; Trang chủ "1 / 5 bài hoàn thành · BÀI TIẾP THEO Giới thiệu bản thân"; rời giữa lượt ⇒ "Thoát lượt làm?" (Ở lại giữ đáp án; Thoát ⇒ mở đầu "Điểm cao nhất 100%" + lịch sử 2 lần). Android/iOS CHƯA VERIFY (VERIFY-DEVICE #26).
